...
deeper_filler<-function(pth, file_names,dest_path,loc_ref=NA, collection_mode="bathymetry map", cross_sectionID=NA, averaging=F, external_GNSS=F){
  if(require(sf)==F){stop("install sf!")}else{
    if (external_GNSS==F){
      if (collection_mode=="bathymetry map"){
      ...
      # update progress bar
        ...
        sc<-read.csv(paste0(pth,file_names[z],".csv"))##read separate files in case are indicated

      
      
     #next is applicable only to deeper sonar file structure      
      ...
      if (collection_mode=="cross-section") { 
        if (is.na(cross_sectionID)){  
          pb <- txtProgressBar(min = 0, max = length(file_names), style = 3) 
          for (z in 1:length(file_names)) {
          Sys.sleep(0.1)
          # update progress bar
          ...
          sc$time_diff<- c(0,abs(diff(sc$time)))
          sc$cross_sectionID<-NA
          ID<-seq(1,length(which(sc$time_diff>=1000))+1)
          IDpos<-c(1,which(sc$time_diff>=1000))
          for (h in 1:length(ID)){
            if(h<length(ID)){ls$cross_sectionID[seq(IDpos[h],IDpos[h+1]-1)]<-ID[h]}
            if(h==length(ID)){ls$cross_sectionID[seq(IDpos[h],nrow(sc))]<-ID[h]}
          ...
          for (t in 1:length(ID)){
            com<-sc[which(sc$cross_sectionID==ID[t]),]
            comp_co<-com[-which(is.na(com$latitude)),]
          #compute effective milliseconds between subsequent coordinates 
          pr<-c()
          for (i in 1:(nrow(comp_co)-1)) {
            pr[i]<-comp_co$time[i+1]-comp_co$time[i]
          ...
          write.csv(ls2,paste0(dest_path,file_names[z],".csv"))
          defsf<-st_as_sf(ls2,coords = c("longitude","latitude"),remove = F)
          st_crs(defsf)<-4326
          write_sf(defsf,paste0(dest_path,file_names[z],"_4326.shp"))
          ...
          for (z in 1:length(file_names)) {
            ...
            ID<-unique(sc[,which(colnames(sc)==cross_sectionID)])            
            sc$nrow<-seq(1:nrow(sc))
            sc$latitude<-ifelse(sc$latitude==0,NA,sc$latitude)
            sc$longitude<-ifelse(sc$longitude==0,NA,sc$longitude)
